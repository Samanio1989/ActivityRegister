...
function benefitText(a) {
  ...
  if (a.benefits.length === 1) return benefitLabel(a.benefits[0]);
  return `เลือกสิทธิ์ได้ 1 จาก ${a.benefits.length} ตัวเลือก`;
}

function benefitLabel(b) {
  return b.hours ? `${b.type} ${b.hours} ชม.` : b.type;
}
...
window.openRegisterModal = function(activityId) {
  ...
  const benefitWrap = $('benefitChoiceWrap');
  const benefitChoices = $('benefitChoices');
  if (benefits.length) {
    benefitWrap.classList.remove('hidden');
    benefitChoices.innerHTML = benefits.map((b, i) => `
      <label class="choice-card">
        <input type="radio" name="selectedBenefit" value="${escapeAttr(b.type)}" ${benefits.length === 1 ? 'checked' : ''} required>
        <span>${escapeHtml(benefitLabel(b))}</span>
      </label>`).join('');
  } else {
    benefitWrap.classList.add('hidden');
    benefitChoices.innerHTML = '';
  }
...
async function submitRegistration(e) {
  ...
  const body = new URLSearchParams({
    ...
    slotId: $('slotId').value || '',
    selectedBenefit: document.querySelector('input[name="selectedBenefit"]:checked')?.value || ''
  });
